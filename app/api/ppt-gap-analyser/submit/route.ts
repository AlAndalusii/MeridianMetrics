...
function getScoreZone(score: number) {
  ...
  return { label: 'Compliant', color: '#059669', zone: 'green', emoji: '🟢' };
}
...
export async function POST(request: Request) {
  try {
    ...
    const strengthsHtml = strengths.length > 0
      ? strengths.map(s => `
        <div style="display:flex;align-items:center;margin:6px 0;gap:10px;">
          ...
          <span style="color:#065f46;font-size:14px;">${s}</span>
        </div>`).join('')
    ...
    // Score bar visual (percentage bar)
...
<body style="font-family:'Helvetica Neue',Arial,sans-serif;background:#f9fafb;margin:0;padding:0;">
  <div style="max-width:600px;margin:0 auto;background:#fff;">
    ...
    <div style="background:linear-gradient(135deg,#064e3b,#065f46,#059669);padding:48px 32px;text-align:center;position:relative;overflow:hidden;">
      ...
      <h1 style="color:#fff;margin:0;font-size:28px;font-weight:800;letter-spacing:-0.5px;">PPT Gap Analyser</h1>
      <p style="color:#6ee7b7;margin:12px 0 0;font-size:15px;">Your Plastic Packaging Tax Compliance Results</p>
    </div>

    <!-- Greeting -->
    <div style="padding:32px 32px 0;">
      <p style="color:#374151;font-size:16px;margin:0 0 8px;">Hi ${leadData.name},</p>
      <p style="color:#6b7280;font-size:14px;margin:0;">Thank you for completing the PPT Gap Analyser. Here are your personalised compliance results for <strong>${leadData.company}</strong>.</p>
    </div>
    ...
    <div style="padding:24px 32px;">
      <div style="background:linear-gradient(135deg,${zone.color}0d,${zone.color}1a);border:2px solid ${zone.color}40;border-radius:20px;padding:32px;text-align:center;">
        <p style="color:${zone.color};font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin:0 0 16px;">PPT Compliance Score</p>
        ...
        <div style="background:#e5e7eb;border-radius:99px;height:12px;margin:0 auto 16px;max-width:300px;overflow:hidden;">
          <div style="background:${scoreBarColor};height:100%;border-radius:99px;width:${scoreBarWidth}%;transition:width 1s ease;"></div>
        </div>
      ...
      <div style="background:#fef2f2;border:2px solid #fecaca;border-radius:12px;padding:20px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr>
            ...
            <td>
              <p style="color:#dc2626;font-weight:700;margin:0 0 6px;font-size:15px;">Critical PPT gaps identified — HMRC enforcement risk</p>
              <p style="color:#991b1b;font-size:13px;margin:0;line-height:1.5;">HMRC is actively auditing PPT-registered businesses. Unresolved gaps can result in back-dated tax demands, penalties up to 100% of tax owed, and criminal prosecution for deliberate non-compliance.</p>
            </td>
          </tr>
        </table>
      </div>
      ...
      <div style="background:#f0fdf4;border:2px solid #bbf7d0;border-radius:12px;padding:20px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr>
            ...
            <td>
              <p style="color:#059669;font-weight:700;margin:0 0 6px;font-size:15px;">You're mostly compliant — let's close the final gaps</p>
              <p style="color:#065f46;font-size:13px;margin:0;line-height:1.5;">Good progress on your PPT compliance! A short consultation can help you address the remaining gaps and ensure you're fully protected before any HMRC review.</p>
            </td>
          </tr>
        </table>
      </div>
    ...
    <div style="padding:0 32px 24px;">
      <h3 style="color:#1f2937;font-size:17px;margin:0 0 16px;font-weight:700;border-bottom:2px solid #f3f4f6;padding-bottom:12px;">
        🚨 Top PPT Compliance Gaps Identified
      </h3>
      ${gapsHtml}
    </div>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td style="padding:12px;background:#f0fdf4;border:2px solid #059669;border-radius:12px;vertical-align:top;">
            ...
            <p style="color:#374151;font-size:13px;margin:0 0 12px;">15-minute consultation to discuss your results and next steps</p>
        ...
        <tr>
          <td style="padding:12px;background:#fff;border:1px solid #e5e7eb;border-radius:12px;vertical-align:top;">
            ...
            <p style="color:#6b7280;font-size:13px;margin:0 0 12px;">• Penalty risk calculation</p>
        ...
        <tr>
          <td style="padding:12px;background:#fff;border:1px solid #e5e7eb;border-radius:12px;vertical-align:top;">
            ...
            <p style="color:#6b7280;font-size:13px;margin:0 0 4px;">• Help sourcing compliant suppliers</p>
    ...
    <div style="padding:0 32px 32px;">
      <div style="background:linear-gradient(135deg,#f0fdf4,#ecfdf5);border-radius:16px;padding:24px;text-align:center;border:1px solid #a7f3d0;">
        <p style="color:#065f46;font-weight:700;margin:0 0 8px;font-size:15px;">Have questions? Contact our PPT specialists</p>
        <p style="color:#374151;font-size:14px;margin:0 0 12px;">
          📞 <a href="tel:+447591092103" style="color:#059669;text-decoration:none;font-weight:600;">+44 7591 092103</a>
          &nbsp;&nbsp;|&nbsp;&nbsp;
...
<body style="font-family:Arial,sans-serif;background:#f9fafb;padding:20px;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
    <div style="background:#065f46;padding:24px;color:#fff;">
      <h2 style="margin:0;font-size:20px;">New PPT Gap Analyser Lead ${zone.emoji}</h2>
    ...
    const batchResult = await resend.batch.send([
      {
        ...
        to: [businessEmail],
        subject: `🆕 PPT Gap Analyser Lead: ${leadData.name} (${leadData.company}) - Score: ${score}/100 [${urgencyLevel} Priority]`,
    ...
    console.error('Error in PPT gap analyzer submit:', error);
