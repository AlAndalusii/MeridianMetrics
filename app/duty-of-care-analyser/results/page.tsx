...
import {
  ...
  ArrowRight, Mail, Building2, User, ClipboardCheck,
...
function ResultsContent() {
  ...
  useEffect(() => {
    try {
      const raw        = localStorage.getItem("doc_gap_analyzer_result")
      const isComplete = localStorage.getItem("doc_gap_analyzer_complete")
      if (!raw || !isComplete) { router.push("/duty-of-care-analyser"); return }
      setResult(JSON.parse(raw))
      if (localStorage.getItem("doc_gap_analyzer_lead_submitted")) setSubmitted(true)
      setIsLoading(false)
      setTimeout(() => setAnimateScore(true), 400)
    } catch { router.push("/duty-of-care-analyser") }
  }, [router])

  const getZone = (score: number) => {
    if (score <= 49) return { label: "High Risk — Significant Duty of Care Exposure", color: "#dc2626", bg: "bg-red-50", border: "border-red-200", text: "text-red-800", badge: "bg-red-100 text-red-700", icon: XCircle,       zone: "red"   }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({ ...leadForm, source: "duty-of-care-analyser", score: result?.score, topGaps: result?.topGaps, urgencyLevel: result?.urgencyLevel }),
      })
      if (res.ok) {
        localStorage.setItem("doc_gap_analyzer_lead_submitted", "true")
        setSubmitted(true)
      } else {
      ...
      <nav className="fixed top-0 w-full bg-white/90 backdrop-blur-xl z-50 border-b border-emerald-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 rounded-full border border-emerald-100">
            ...
            <span className="text-sm poppins-medium text-emerald-700">Duty of Care Results</span>
          </div>
        </div>
      </nav>
      ...
      <div className="pt-24 pb-16 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className={`rounded-3xl p-8 md:p-12 border-2 ${zone.border} ${zone.bg} mb-8 text-center shadow-lg`}>
            ...
            <h1 className={`poppins-bold text-3xl md:text-4xl ${zone.text} mb-3`}>
              Your Duty of Care Score
            </h1>
            <p className={`poppins-regular text-base ${zone.text} opacity-80 max-w-lg mx-auto`}>
              {result.score <= 49
                ? "Your Duty of Care compliance has critical gaps. The Environment Agency issues £300 fixed penalties on-the-spot and can prosecute for serious breaches. Action is needed urgently."
                : result.score <= 79
                ? "You have some Duty of Care practices in place, but gaps remain. These need addressing before an inspection — the cost of a fine far exceeds the cost of fixing the paperwork."
                : "Your Duty of Care compliance is in good shape. A few minor improvements will give you full confidence during any EA or council inspection."}
            </p>
          </div>
          ...
          {/* Lead form / CTA */}
          {!submitted ? (
            <div className="bg-gradient-to-br from-emerald-900 to-emerald-950 rounded-3xl p-8 md:p-12 text-white shadow-2xl" ref={leadFormRef}>
              {!showLeadForm ? (
                <div className="text-center">
                  ...
                  <p className="poppins-regular text-emerald-200/70 mb-8 max-w-md mx-auto leading-relaxed">
                    Receive a personalised Duty of Care gap report with specific actions, document templates and a 48hr audit quote — free, no obligation.
                  </p>
                  <div className="flex flex-wrap gap-3 justify-center mb-8">
                    {["Written gap analysis", "Action priority list", "WTN template included", "48hr audit quote"].map(f => (
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input value={leadForm.company} onChange={e => setLeadForm(p => ({ ...p, company: e.target.value }))}
                          placeholder="Care home / agency name" className="pl-10 bg-emerald-800/40 border-emerald-700/50 text-white placeholder:text-emerald-500 focus:border-emerald-400" />
                      </div>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-6 text-center">
            {[
              { icon: BadgeCheck, label: "Independent Experts" },
              { icon: Star,       label: "48hr Reports" },
              { icon: Lock,       label: "No Data Sharing" },
            ].map(({ icon: I, label }) => (
              <div key={label} className="flex items-center gap-2 text-emerald-600">
                <I className="w-4 h-4" />
                <span className="poppins-medium text-xs">{label}</span>
              </div>
            ))}
          </div>
        </div>
