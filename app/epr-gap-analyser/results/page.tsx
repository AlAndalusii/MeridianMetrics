  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width={radius * 2} height={radius * 2} className="-rotate-90">
        ...
        <circle
          cx={radius} cy={radius} r={normalizedRadius} fill="none" stroke={color} strokeWidth={stroke}
...
function ResultsContent() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("epr_gap_analyser_result")
      const isComplete = localStorage.getItem("epr_gap_analyser_complete")

      if (!raw || !isComplete) {
        router.push("/epr-gap-analyser")
        return
      }
      ...
      const alreadySubmitted = localStorage.getItem("epr_gap_analyser_lead_submitted")
  ...
  const getZone = (score: number) => {
    if (score <= 49) return { label: "High Risk — Significant EPR Exposure", color: "#dc2626", bg: "bg-red-50", border: "border-red-200", text: "text-red-800", badge: "bg-red-100 text-red-700", icon: XCircle, zone: "red" }
    ...
    return { label: "Compliant — Minor Improvements Needed", color: "#2563eb", bg: "bg-blue-50", border: "border-blue-200", text: "text-blue-800", badge: "bg-blue-100 text-blue-700", icon: CheckCircle, zone: "green" }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const answers = (() => {
        try {
          const raw = localStorage.getItem("epr_gap_analyser_answers")
          return raw ? Object.entries(JSON.parse(raw)).map(([section, answer]) => ({
            section, question: section, answer: answer as string,
          })) : []
        } catch { return [] }
      })()

      const res = await fetch("/api/epr-gap-analyser/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ leadData: leadForm, ...result, answers }),
      })
      ...
      localStorage.setItem("epr_gap_analyser_lead_submitted", "true")
      setSubmitted(true)
    } catch (err) {
  ...
  const handleResend = () => {
    localStorage.removeItem("epr_gap_analyser_lead_submitted")
    setSubmitted(false)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="poppins-medium text-blue-700">Calculating your EPR compliance score…</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 relative overflow-hidden">
      ...
      {/* Nav */}
      <nav className="sticky top-0 w-full bg-white/90 backdrop-blur-xl z-50 border-b border-blue-100 shadow-sm">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-8">
          ...
          <div className="flex items-center justify-center gap-6 mb-6">
            {[
              ...
              <div key={z} className={`flex flex-col items-center gap-1 transition-all ${active ? "scale-125" : "opacity-40"}`}>
                <div
                  className={`w-5 h-5 rounded-full ${active ? "ring-2 ring-offset-2" : ""} ${z === "red" ? "ring-red-500" : z === "amber" ? "ring-amber-500" : "ring-blue-500"}`}
                  style={{ background: z === "red" ? "#dc2626" : z === "amber" ? "#d97706" : "#2563eb" }}
                />
        ...
        {isHighPriority ? (
          <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-3xl p-8 text-white shadow-xl shadow-red-500/20">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="poppins-bold text-lg text-red-100 mb-1">Critical EPR gaps identified — avoid penalties</p>
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-8 text-white shadow-xl shadow-blue-500/20">
            <div className="flex items-start gap-3 mb-4">
              <CheckCircle className="w-7 h-7 text-blue-200 flex-shrink-0 mt-0.5" />
              <div>
                <p className="poppins-bold text-lg text-blue-100 mb-1">You&apos;re mostly compliant — let&apos;s close the final gaps</p>
            ...
            <p className="text-center text-xs text-blue-300 mt-3 poppins-regular">No obligation — 20-minute call with an EPR compliance expert</p>
          </div>
        )}

        {/* Recommended For You */}
        <div className="bg-white/85 backdrop-blur-xl rounded-3xl p-8 border border-blue-100 shadow-md">
          <h3 className="poppins-bold text-xl text-blue-900 mb-2 flex items-center gap-2">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 mb-6">
            <h4 className="poppins-bold text-blue-900 mb-1">Free 15-Minute Consultation</h4>
            <p className="poppins-regular text-sm text-blue-700 mb-4">Get personalised advice on your EPR compliance gaps</p>
            <ul className="space-y-2 mb-5">
              {["Discuss your results", "Calculate fee overpayment risk", "Create action plan"].map((item, i) => (
                <li key={i} className="flex items-center gap-2 text-sm poppins-medium text-blue-800">
            ...
            <div className="border border-gray-200 rounded-2xl p-5 bg-white">
              ...
              <ul className="space-y-1 mb-4">
                {["30-day email/phone support", "Help correcting EPR data submissions", "Record-keeping templates", "Follow-up compliance check"].map((f, i) => (
                  <li key={i} className="flex items-center gap-2 text-xs poppins-medium text-gray-700">
                    <CheckCircle className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />{f}
                  </li>
                ))}
              </ul>
        ...
        {/* Breakdown */}
        <div className="bg-white/85 backdrop-blur-xl rounded-3xl p-8 border border-blue-100 shadow-md">
          <h3 className="poppins-bold text-xl text-blue-900 mb-6 flex items-center gap-2">
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {result.topGaps.map((gap, i) => (
                  <div key={i} className="flex gap-4 p-4 bg-amber-50 border border-amber-100 rounded-2xl">
                    ...
                    <div className="flex-1">
                      ...
                      {gap.risk && (
                        <p className="poppins-medium text-xs text-red-600 mt-1.5 flex items-center gap-1">
                          <XCircle className="w-3 h-3" />{gap.risk}
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="space-y-2">
                {result.strengths.map((s, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-blue-50 border border-blue-100 rounded-xl">
                    ...
                    <span className="poppins-medium text-sm text-blue-800">{s}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Lead Capture */}
        <div ref={leadFormRef} className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 border border-blue-100 shadow-md">
          {submitted ? (
            <div className="text-center py-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center mb-5">
                ...
                <Link href="/"><Button variant="outline" className="poppins-semibold border-2 border-blue-200 text-blue-700 hover:bg-blue-50 py-4 px-6 rounded-2xl w-full sm:w-auto">Back to Home</Button></Link>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 border-t border-blue-100">
                <button
                  onClick={handleResend}
                  className="text-sm poppins-medium text-blue-700 hover:text-blue-900 hover:underline transition-colors flex items-center gap-1.5"
                >
                  <Mail className="w-3.5 h-3.5" />
                  Send to a different email
                </button>
                <span className="hidden sm:block text-gray-300">·</span>
                <Link href="/epr-gap-analyser" className="text-sm poppins-medium text-gray-500 hover:text-gray-700 hover:underline transition-colors flex items-center gap-1.5">
            ...
            <>
              <div className="flex items-start gap-3 mb-6">
                ...
                <div>
                  <h3 className="poppins-bold text-xl text-blue-900">Get Your EPR Compliance Report</h3>
              ...
              {!showLeadForm ? (
                <div className="text-center">
                  <Button onClick={scrollToLeadForm} className="poppins-bold py-5 px-10 text-base bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl hover:shadow-xl hover:shadow-blue-500/30 hover:scale-105 transition-all group">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <User className="w-4 h-4 inline mr-1.5 text-blue-600" />Full Name *
                      </label>
                      <Input type="text" value={leadForm.name} onChange={e => setLeadForm(f => ({ ...f, name: e.target.value }))} placeholder="Jane Smith"
                        className={`rounded-xl border-2 py-3 poppins-regular ${formErrors.name ? "border-red-300 bg-red-50" : "border-blue-200 focus:border-blue-500"}`} />
                      {formErrors.name && <p className="text-xs text-red-600 mt-1 poppins-medium">{formErrors.name}</p>}
                    </div>
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <Mail className="w-4 h-4 inline mr-1.5 text-blue-600" />Work Email *
                      </label>
                      <Input type="email" value={leadForm.email} onChange={e => setLeadForm(f => ({ ...f, email: e.target.value }))} placeholder="jane@company.com"
                        className={`rounded-xl border-2 py-3 poppins-regular ${formErrors.email ? "border-red-300 bg-red-50" : "border-blue-200 focus:border-blue-500"}`} />
                      {formErrors.email && <p className="text-xs text-red-600 mt-1 poppins-medium">{formErrors.email}</p>}
                    </div>
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <Building2 className="w-4 h-4 inline mr-1.5 text-blue-600" />Company Name *
                      </label>
                      <Input type="text" value={leadForm.company} onChange={e => setLeadForm(f => ({ ...f, company: e.target.value }))} placeholder="Acme Ltd"
                        className={`rounded-xl border-2 py-3 poppins-regular ${formErrors.company ? "border-red-300 bg-red-50" : "border-blue-200 focus:border-blue-500"}`} />
                      {formErrors.company && <p className="text-xs text-red-600 mt-1 poppins-medium">{formErrors.company}</p>}
                    </div>
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <Phone className="w-4 h-4 inline mr-1.5 text-blue-600" />Phone (optional)
                      </label>
                      <Input type="tel" value={leadForm.phone} onChange={e => setLeadForm(f => ({ ...f, phone: e.target.value }))} placeholder="+44 7XXX XXXXXX"
                        className="rounded-xl border-2 border-blue-200 focus:border-blue-500 py-3 poppins-regular" />
                    </div>
                  </div>
                  ...
                  <label className={`flex items-start gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${leadForm.gdprConsent ? "border-blue-300 bg-blue-50" : formErrors.gdprConsent ? "border-red-200 bg-red-50" : "border-gray-200 bg-gray-50 hover:border-blue-200"}`}>
                    <div onClick={() => setLeadForm(f => ({ ...f, gdprConsent: !f.gdprConsent }))}
                      className={`flex-shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center mt-0.5 transition-all ${leadForm.gdprConsent ? "border-blue-500 bg-blue-500" : "border-gray-300 bg-white"}`}>
                    ...
                    <span className="poppins-regular text-xs text-gray-600 leading-relaxed">
                      I consent to Millstone Compliance processing my data to send me my EPR compliance report and relevant compliance updates. I can unsubscribe at any time.{" "}
                  ...
                  <Button onClick={handleSubmit} disabled={isSubmitting}
                    className="w-full poppins-bold py-5 text-base bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl hover:shadow-xl hover:shadow-blue-500/30 transition-all group disabled:opacity-60">
                    {isSubmitting ? (
        ...
        <div className="flex flex-col sm:flex-row gap-3 pb-6">
          <Link href="/epr-gap-analyser" className="flex-1">
            <Button variant="outline" className="w-full poppins-semibold border-2 border-blue-200 text-blue-700 hover:bg-blue-50 rounded-2xl py-4">Retake Assessment</Button>
          </Link>
          <Link href="/" className="flex-1">
            <Button variant="outline" className="w-full poppins-semibold border-2 border-gray-200 text-gray-600 hover:bg-gray-50 rounded-2xl py-4">Back to Home</Button>
          </Link>
        </div>
        ...
        <div className="text-center pb-8">
          <div className="flex items-center justify-center gap-2 text-xs text-gray-400 poppins-regular">
            ...
            Millstone Compliance — UK EPR Packaging Specialists
          </div>
        </div>
      </div>
      </div>
