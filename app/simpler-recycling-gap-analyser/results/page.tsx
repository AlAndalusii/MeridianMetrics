  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center">
        ...
        <span className="poppins-medium text-sm text-gray-500">/10</span>
      </div>
    </div>
  )
}
...
function ResultsContent() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("gap_analyzer_result")
      const isComplete = localStorage.getItem("gap_analyzer_complete")

      if (!raw || !isComplete) {
        router.push("/simpler-recycling-gap-analyser")
        return
      }
      ...
      const alreadySubmitted = localStorage.getItem("gap_analyzer_lead_submitted")
  ...
  const getZone = (score: number) => {
    if (score <= 3) return { label: "Critical Non-Compliance", color: "#dc2626", bg: "bg-red-50", border: "border-red-200", text: "text-red-800", badge: "bg-red-100 text-red-700", icon: XCircle, zone: "red" }
    if (score <= 6) return { label: "Partial Compliance — Major Gaps", color: "#d97706", bg: "bg-amber-50", border: "border-amber-200", text: "text-amber-800", badge: "bg-amber-100 text-amber-700", icon: AlertTriangle, zone: "amber" }
    return { label: "Good Compliance — Minor Improvements", color: "#059669", bg: "bg-green-50", border: "border-green-200", text: "text-green-800", badge: "bg-green-100 text-green-700", icon: CheckCircle, zone: "green" }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const answers = (() => {
        try {
          const raw = localStorage.getItem("gap_analyzer_answers")
      ...
      const res = await fetch("/api/gap-analyser/submit", {
        method: "POST",
      ...
      localStorage.setItem("gap_analyzer_lead_submitted", "true")
      setSubmitted(true)
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="poppins-medium text-green-700">Calculating your compliance score…</p>
        </div>
      </div>
    )
  }
  ...
  const isHighPriority = result.score < 7

  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 relative overflow-hidden">
      {/* Background blobs */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-green-200/20 rounded-full blur-3xl animate-pulse" />
        <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-emerald-200/20 rounded-full blur-3xl animate-pulse delay-1000" />
      </div>

      {/* Nav */}
      <nav className="sticky top-0 w-full bg-white/90 backdrop-blur-xl z-50 border-b border-green-100 shadow-sm">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        ...
        <div className={`${zone.bg} ${zone.border} border-2 rounded-3xl p-8 md:p-10 text-center shadow-lg`}>
          ...
          <div className="flex items-center justify-center gap-6 mb-6">
            {[
              { zone: "red", label: "0–3", active: result.score <= 3 },
              { zone: "amber", label: "4–6", active: result.score >= 4 && result.score <= 6 },
              { zone: "green", label: "7–10", active: result.score >= 7 },
        ...
        {isHighPriority ? (
          <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-3xl p-8 text-white shadow-xl shadow-red-500/20">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="poppins-bold text-lg text-red-100 mb-1">Critical gaps identified — avoid penalties</p>
                <p className="poppins-regular text-sm text-red-200">Non-compliance with Simpler Recycling can result in enforcement notices and fines. The Environment Agency is actively auditing businesses.</p>
              </div>
            </div>
            <Button
              onClick={() => setShowCalendly(true)}
          ...
          <div className="bg-gradient-to-br from-green-600 to-emerald-700 rounded-3xl p-8 text-white shadow-xl shadow-green-500/20">
            <div className="flex items-start gap-3 mb-4">
              <CheckCircle className="w-7 h-7 text-green-200 flex-shrink-0 mt-0.5" />
              <div>
                <p className="poppins-bold text-lg text-green-100 mb-1">You&apos;re mostly compliant — let&apos;s fix the final gaps</p>
                <p className="poppins-regular text-sm text-green-200">A short consultation will help you close the remaining gaps and ensure you&apos;re fully protected before any inspection.</p>
              </div>
            </div>
            <Button
              onClick={() => setShowCalendly(true)}
            ...
            <p className="text-center text-xs text-green-300 mt-3 poppins-regular">No obligation — just a 20-minute call with a compliance expert</p>
          </div>
        )}

        {/* Score breakdown */}
        <div className="bg-white/85 backdrop-blur-xl rounded-3xl p-8 border border-green-100 shadow-md">
          <h3 className="poppins-bold text-xl text-green-900 mb-6 flex items-center gap-2">
              ...
              <div className="space-y-2">
                {result.strengths.map((s, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-green-50 border border-green-100 rounded-xl">
                    ...
                    <span className="poppins-medium text-sm text-green-800">{s}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Lead Capture Form */}
        <div ref={leadFormRef} className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 border border-green-100 shadow-md">
          {submitted ? (
            ...
            <>
              <div className="flex items-start gap-3 mb-6">
                ...
                <div>
                  <h3 className="poppins-bold text-xl text-green-900">Get Your Compliance Score Report</h3>
              ...
              {!showLeadForm ? (
                <div className="text-center">
                  <Button
                    ...
                    className="poppins-bold py-5 px-10 text-base bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-2xl hover:shadow-xl hover:shadow-green-500/30 hover:scale-105 transition-all group"
                  >
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <User className="w-4 h-4 inline mr-1.5 text-green-600" />Full Name *
                      </label>
                      <Input
                        type="text"
                        value={leadForm.name}
                        ...
                        className={`rounded-xl border-2 py-3 poppins-regular ${formErrors.name ? "border-red-300 bg-red-50" : "border-green-200 focus:border-green-500"}`}
                      />
                      {formErrors.name && <p className="text-xs text-red-600 mt-1 poppins-medium">{formErrors.name}</p>}
                    </div>
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <Mail className="w-4 h-4 inline mr-1.5 text-green-600" />Work Email *
                      </label>
                      <Input
                        type="email"
                        ...
                        className={`rounded-xl border-2 py-3 poppins-regular ${formErrors.email ? "border-red-300 bg-red-50" : "border-green-200 focus:border-green-500"}`}
                      />
                      {formErrors.email && <p className="text-xs text-red-600 mt-1 poppins-medium">{formErrors.email}</p>}
                    </div>
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <Building2 className="w-4 h-4 inline mr-1.5 text-green-600" />Company Name *
                      </label>
                      <Input
                        type="text"
                        ...
                        className={`rounded-xl border-2 py-3 poppins-regular ${formErrors.company ? "border-red-300 bg-red-50" : "border-green-200 focus:border-green-500"}`}
                      />
                      {formErrors.company && <p className="text-xs text-red-600 mt-1 poppins-medium">{formErrors.company}</p>}
                    </div>
                    <div>
                      <label className="block text-sm poppins-semibold text-gray-700 mb-1.5">
                        <Phone className="w-4 h-4 inline mr-1.5 text-green-600" />Phone (optional)
                      </label>
                      <Input
                        type="tel"
                        ...
                        className="rounded-xl border-2 border-green-200 focus:border-green-500 py-3 poppins-regular"
                      />
                    </div>
                  </div>
                  ...
                  {/* GDPR */}
                  <label className={`flex items-start gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${leadForm.gdprConsent ? "border-green-300 bg-green-50" : formErrors.gdprConsent ? "border-red-200 bg-red-50" : "border-gray-200 bg-gray-50 hover:border-green-200"}`}>
                    <div
                      onClick={() => setLeadForm(f => ({ ...f, gdprConsent: !f.gdprConsent }))}
                      className={`flex-shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center mt-0.5 transition-all ${leadForm.gdprConsent ? "border-green-500 bg-green-500" : "border-gray-300 bg-white"}`}
                    >
                    ...
                    <span className="poppins-regular text-xs text-gray-600 leading-relaxed">
                      I consent to Millstone Compliance processing my data to send me my compliance report and relevant compliance updates. I can unsubscribe at any time.{" "}
                  ...
                  <Button
                    ...
                    className="w-full poppins-bold py-5 text-base bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-2xl hover:shadow-xl hover:shadow-green-500/30 transition-all group disabled:opacity-60"
                  >
                    {isSubmitting ? (
        ...
        <div className="flex flex-col sm:flex-row gap-3 pb-6">
          <Link href="/simpler-recycling-gap-analyser" className="flex-1">
            <Button variant="outline" className="w-full poppins-semibold border-2 border-green-200 text-green-700 hover:bg-green-50 rounded-2xl py-4">
              Retake Assessment
            </Button>
          </Link>
        ...
        <div className="text-center pb-8">
          <div className="flex items-center justify-center gap-2 text-xs text-gray-400 poppins-regular">
            ...
            Millstone Compliance — UK Simpler Recycling Specialists
          </div>
        </div>
      </div>
